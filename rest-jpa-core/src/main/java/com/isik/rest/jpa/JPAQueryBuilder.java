...
public class JPAQueryBuilder {
    ...
    /**
     *
     * @param entityManager entity manager for clazz
     * @param queryModel query object
     * @param clazz destination filtered JPA Entity class
     * @param <T>
     * @return
     */
    public static <T> QueryResult<T> filter(EntityManager entityManager, QueryModel queryModel, Class<T> clazz) {
        ...
        try {
            ...
            //apply filters
            List<Predicate> predicates = getPredicates(queryModel, builder, root, clazz);
            if(!predicates.isEmpty()){
                query.where(predicates.toArray(new Predicate[]{}));
            }
            //apply orders
            List<Order> orders = getOrders(queryModel, builder, root, clazz);
            if (!orders.isEmpty()){
                query.orderBy(orders);
            }
            //apply paging
            TypedQuery<T> typedQuery = entityManager.createQuery(query);
            typedQuery.setMaxResults(queryModel.getPageSize());
            typedQuery.setFirstResult(queryModel.getCurrentPage() * queryModel.getPageSize());
            //set results
            result.setResults(typedQuery.getResultList());
            //set total count
            result.setTotalCount(getTotalCount(entityManager, queryModel, clazz,builder));
        } catch (Exception e) {
            LOGGER.error("",e);
        }
        return result;
    ...
    private static <T> long getTotalCount(EntityManager entityManager, QueryModel filter, Class<T> clazz, CriteriaBuilder builder) throws NoSuchFieldException {
    ...
    private static <T> List<Order> getOrders(QueryModel tableQuery, CriteriaBuilder cb, Root<T> root, Class<T> rootClazz) throws NoSuchFieldException {
    ...
    private static <T> List<Predicate> getPredicates(QueryModel tableQuery, CriteriaBuilder cb, Root<T> root, Class<T> clazz) throws NoSuchFieldException {
